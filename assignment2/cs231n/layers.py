...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    if mode == 'train':
        ...
        cache={'x':x,'mean':mean,'std':std,'gamma':gamma,'z':z,'var':var,'axis':layernorm,'eps':eps}
...
def batchnorm_backward(dout, cache):
    ...
    # cache={'x':x,'mean':mean,'std':std,'gamma':gamma,'z':z,'var':var,'axis':layernorm}
    x = cache['x']
    mean = cache['mean']
    std = cache['std']
    gamma = cache['gamma']
    ...
    dz = dout*gamma     #N,D
    dzdx = dz/std       #N,D
    divar = np.sum(dz*xmu,axis=0)   #D,
    dsqrtvar = -divar/(std**2)    #D,
    dvar = 0.5*dsqrtvar/np.sqrt(var+cache['eps'])     
    dsq = (np.ones((N,D))*dvar)/N
    dmu2 = 2*xmu*dsq
    dx1 = dzdx + dmu2
    dmu = -1*np.sum(dx1,axis=0)
    dx = dx1 + ((np.ones((N,D))*dmu)/N)
    # *****END OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
...
def batchnorm_backward_alt(dout, cache):
    ...
    dgamma = np.sum(dout * xhat, axis=cache['axis'])
    N = dout.shape[0]
    dxhat = dout * cache['gamma']                                    #[NxD]
    dxhat_sum = np.sum(dxhat,axis=0)                                  #[1xD]
    dx = dxhat - dxhat_sum/N - np.sum(dxhat * xhat,axis=0) * xhat/N          #[NxD]
    dx /= cache['std']
    # *****END OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    ###########################################################################
    #                             END OF YOUR CODE                            #
...
def layernorm_forward(x, gamma, beta, ln_param):
    ...
    ln_param['mode'] = 'train' # always train mode
...
def conv_forward_naive(x, w, b, conv_param):
    ...
    # create w_row matrix
    w_row = w.reshape(F, -1)                            #[F x C*FH*FW]
    ...
    for index in range(N):
        ...
        for i in range(0, H_pad-FH+1, stride):
            for j in range(0, W_pad-FW+1,stride):
                x_col[:,neuron] = x_pad[index,:,i:i+FH,j:j+FW].reshape(-1)
                neuron += 1
    ...
    for index in range(N):
        ...
        for i in range(0, H - PH + 1, stride):
            for j in range(0, W - PW + 1, stride):
                pool_region = x[index,:,i:i+PH,j:j+PW].reshape(C,-1)
...
def max_pool_backward_naive(dout, cache):
    ...
    dx = np.zeros(x.shape)
    for index in range(N):
        dout_row = dout[index].reshape(C, -1)
        neuron = 0
        for i in range(0, H-PH+1, stride):
            for j in range(0, W-PW+1, stride):
                pool_region = x[index,:,i:i+PH,j:j+PW].reshape(C,-1)
                pool_indices = pool_region.argmax(axis=1)
                dout_cur = dout_row[:,neuron]
                dmax_pool = np.zeros(pool_region.shape)
                dmax_pool[np.arange(C), pool_indices] = dout_cur
...
def spatial_groupnorm_backward(dout, cache):
    ...
    # from batch normalization backward alt
