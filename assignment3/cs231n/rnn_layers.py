...
def rnn_backward(dh, cache):
    ...
    ##############################################################################
    N, T, H = dh.shape
    dxl, dprev_h, dWx, dWh, db = rnn_step_backward(
        dh[:, T - 1, :], cache[T - 1])
    D = dxl.shape[1]
    dx = np.zeros((N, T, D))
    dx[:, T - 1, :] = dxl
    for i in range(T - 2, -1, -1):
        dxc, dprev_hc, dWxc, dWhc, dbc = rnn_step_backward(
...
def lstm_step_forward(x, prev_h, prev_c, Wx, Wh, b):
    ...
    i, f, o, g = h[:, :H], h[:, H:2 * H], h[:,
                                            2 * H:3 * H], h[:, 3 * H:4 * H]  # [NxH]
...
def lstm_forward(x, h0, Wx, Wh, b):
    ...
    cache = []
    for i in range(T):
        next_h, next_c, cache_i = lstm_step_forward(
